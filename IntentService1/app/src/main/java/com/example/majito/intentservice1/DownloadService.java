...
/**
 * Created by Majito on 20/08/2017.
 */
...
public class DownloadService extends IntentService {
    ...
    private String[] parseResult(String result) {
        ...
        String[] genresDescription = null;
        try {
            JSONObject response = new JSONObject(result);
            JSONArray genres = response.optJSONArray("genres");
            genresDescription = new String[genres.length()];

            for (int i = 0; i < genres.length(); i++) {
                JSONObject post = genres.optJSONObject(i);
                String description = post.optString("description");
                genresDescription[i] = description;
            }
